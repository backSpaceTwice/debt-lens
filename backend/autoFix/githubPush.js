...
/** Collapse [1,2,3,5,6] → "1-3, 5-6" for a readable changed-lines summary. */
function toRanges(nums) {
  const s = [...new Set(nums)].filter(Number.isInteger).sort((a, b) => a - b);
  if (!s.length) return '';
  const out = [];
  let start = s[0], end = s[0];
  for (let i = 1; i < s.length; i++) {
    if (s[i] === end + 1) end = s[i];
    else { out.push(start === end ? `${start}` : `${start}-${end}`); start = end = s[i]; }
  }
...
/**
 * Build the commit message. GitHub pre-fills a single-commit PR's title from the
 * first line and its description from the rest, so this becomes the PR title +
 * body: a concise title, then the summary, the grounded changed lines, the
 * confidence, and a reviewer checklist.
 */
function buildCommitMessage(fix) {
  const lines = [`fix(debtlens): auto-fix in ${fix.file}`, ''];
  if (fix.fixSummary) lines.push(fix.fixSummary, '');

  const ranges = toRanges(fix.changedLineRefs || []);
  if (ranges) lines.push(`**Changed lines:** ${ranges}`);
  if (typeof fix.confidence === 'number') lines.push(`**Confidence:** ${fix.confidence}%`);

  if (fix.verificationSteps?.length) {
    lines.push('', '**Reviewer checklist:**');
    for (const step of fix.verificationSteps) lines.push(`- [ ] ${step}`);
  }

  lines.push('', '---', '🤖 Generated by DebtLens auto-fix — review before merging.');
  return lines.join('\n');
}
...
async function pushViaApi({ token, owner, repo, branch, filePath, content, fix }) {
  ...
  const putBody = {
    message: buildCommitMessage(fix),
...
export async function pushFixToGitHub({ repoPath, branch, owner, repo, fix }) {
  ...
  let result;
  if (!token) {
    result = download('No GitHub token set (DEBTLENS_GITHUB_TOKEN or GITHUB_TOKEN).');
  } else {
    try {
